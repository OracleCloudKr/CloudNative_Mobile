...
4. 변경하기 전에 가져 오기 중에 사전 빌드 된 내용을 검토 합니다. **ProcessOffer**라는 첫 번째 노드를 클릭하여 선택하고 팝업 메뉴에서 `편집` `펜`버튼을 클릭하십시오. 

![](images/303/04.integration.processoffer.png)


5. **Oracle REST Endpoint 구성**대화 상자 창이 표시됩니다. 이 페이지는 임포트 된 통합 플로우의 인터페이스, 즉 적절한 JSON 형식으로 REST 요청 및 응답을 통해 이 프로세스와 상호 작용하는 방법을 요약합니다. 나머지 세부 사항을 보려면 오른쪽 상단에있는 &lt;뒤로> 버튼을 클릭하십시오. 
